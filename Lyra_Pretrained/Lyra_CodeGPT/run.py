...
from dataset import lyraDataset
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    dataset = lyraDataset(tokenizer, args, logger, file_type='valid' if evaluate else 'train',
...
def train(args, train_dataset, model, tokenizer, fh, pool):
    ...
    # Prepare Data and train numbers
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        for step, (batch, token_labels) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                scheduler.step()  
                global_step += 1
                # Logging and write loss infromation
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        results = eval_bleu(args, model, tokenizer, file_type='valid',beam_size=1)
                        for key, value in results.items():
                            if key in ['rouge-l','bleu','excutable_rate','AST_same_without_str_rate','func_correctness_rate']:
                                if type(value)==dict:
                                    tb_writer.add_scalar('eval_{}'.format(key), value['f'], global_step)
                                    logger.info("  %s = %s", key, round(value['f'],4))
                                else:
                        ...
                        output_dir = os.path.join(args.output_dir, '{}-{}-{}'.format(checkpoint_prefix, global_step, round(results['func_correctness_rate'],4)))
                    else:
...
def eval_bleu(args, model, tokenizer, file_type='test', num=200, beam_size = 10):
    dataset = lyraDataset(tokenizer, args, logger, file_type=file_type, block_size=args.block_size, mode='test')
    ...
    for step, (batch, token_labels) in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            # logger.info(f"inputs.shape[0]: {inputs.shape[0]}")
    ...
    golds = datas['code'].tolist()[:num]
    
    assert len(preds) == len(golds)
    result_dict = get_rouge_dict(preds, golds)
    result_dict['bleu'] = get_bleu4_score(preds, golds, tokenizer)
    result_dict['excutable_rate'] = get_executable_rate(preds)
    rate, index = get_func_correctness(preds, golds, repalce_string=True, need_index=True)
    result_dict['AST_same_without_str_rate'] = rate
    result_dict['exact_without_str_index'] = index
    rate , index  = get_func_correctness(preds, golds, need_index=True)
    result_dict['func_correctness_rate'] = rate
    result_dict['exact_index'] = index

    with open(os.path.join(args.output_dir, f"{file_type}.result_dict.txt"), 'w') as f:
        json.dump(result_dict, f, indent=4)

    with open(os.path.join(args.output_dir,file_type+".test.output"),'a') as f, open(os.path.join(args.output_dir,file_type+".test.gold"),'a') as f1:
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--nl_langs", default='en', type=str, 
                        help="Languages to train, if all, train all languages in data_dir")
    parser.add_argument("--max_gen_len", default=512, type=int, 
                        help="Languages to train, if all, train all languages in data_dir")
    ...
    if args.do_eval:            # only works on 1 GPU
        result_dict = eval_bleu(args, model, tokenizer, file_type='valid', num=200)
        logger.info(f"dev result: {result_dict}")

    if args.do_infer:            # only works on 1 GPU
        result_dict = eval_bleu(args, model, tokenizer, file_type='test', num=200)
        logger.info(f"test result: {result_dict}")
